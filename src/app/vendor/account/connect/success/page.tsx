...
import { callBackConnectAccountSuccess } from '@/services/vendor';
...
const AccountConnectedSuccess = () => {
    ...
    const callBackTo = async (code: string, state: string) => {
        try {
            const response = await callBackConnectAccountSuccess({ code, state });
    ...
    useEffect(() => {
        ...
        if (code && state) {
            console.log('call Apiiii', code, state)
            callBackTo(code, state)
        };
    }, []);
    return (
        <div
